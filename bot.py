...
async def init_db():
    ...
    try:
        ...
        if not column_exists:
            await conn.execute("ALTER TABLE items ADD COLUMN status TEXT DEFAULT '🛒 Выкуплен'")
...
async def show_faq(call: CallbackQuery):
    faq_text = (
        ...
        "1️⃣ <b>Как рассчитывается процент вклада?</b>\n"
        "Процент вклада рассчитывается по формуле:\n"
        "<code>Сумма вашего вклада / сумма всех вкладчиков</code>\n"
        "20% с суммы вашего чистого заработка с поставки идёт на поддержание бота, обеспечение услуг доставки, продажи и поиска вещей, которые ГАРАНТИРОВАННО продадутся с наценкой.\n\n"

        "2️⃣ <b>Как часто бывают поставки?</b>\n"
        ...
        "3️⃣ <b>Сколько я получу, если вложу 10 000 рублей?</b>\n"
        "Примерный заработок оценивается в 20–100% от изначальной стоимости.\n\n"

        "4️⃣ <b>Что делать, если процент вклада уменьшается?</b>\n"
...
async def view_supply(call: CallbackQuery):
    buttons = [
        [InlineKeyboardButton(text="🚚 Текущая поставка", callback_data="supply_list_current")],
...
STATUSES = [
    "🛒 Выкуплен",
    "📦 Едет на склад в Китае",
    "🔄 Ждет отправки в РФ",
    "✈️ Едет в РФ",
    "📬 Ждет отправки админу",
    "✅ Получен",
...
async def make_contribution_start(call: CallbackQuery, state: FSMContext):
    ...
    success_text = (
        "✅ <b>Ваш вклад принят!</b>\n\n"
        "💬 В течение нескольких часов администратор проверит платеж и начислит вклад на ваш аккаунт.\n\n"
        "📌 Вы можете оплатить по следующим реквизитам:\n\n"
        "💳 <b>Карта:</b> <code>2200 7010 4325 8000</code>\n"
        "📞 <b>Телефон:</b> <code>+79219753645</code>\n"
        "🏦 <b>Банк:</b> Т-Банк\n"
        "💬 <b>Комментарий:</b> Вклад в поставку"
    )
    ...
    await call.message.edit_text(success_text, reply_markup=markup, parse_mode="HTML")
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                ...
                f"Нажал «Сделать вклад»\n"
                f"Ожидает начисления.",
                parse_mode="HTML"
            )
        except Exception as e:
